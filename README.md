...
- `scripts/diagnose_ground_dcr.py` — проверка идентифицируемости цветового
  атмосферного члена с фиксированными эффектами звёзд и кадров;
- `scripts/diagnose_ground_frame_influence.py` — leave-one-frame-out проверка,
  выявляющая отдельные кадры с непропорциональным влиянием на полный стек;
...
Конвейер намеренно разделяет три уровня данных. Исходное наземное положение
астероида измеряется только по нашим FITS-кадрам. Каталог Gaia DR3 предоставляет
координаты опорных звёзд, необходимые для перевода пикселей в RA/Dec; это
астрометрическая система отсчёта, а не дополнительное изображение астероида.
Gaia FPR содержит отдельные космические наблюдения самого астероида на других
эпохах и входит только в последующий совместный орбитальный fit. Наземные
таблицы сохраняются отдельно, а FPR-прогноз не используется как prior на центр
астероида: иначе можно было бы искусственно уменьшить O−C и поглотить искомое
колебание фотоцентра.
...
.venv/bin/python scripts/diagnose_ground_dcr.py
.venv/bin/python scripts/diagnose_ground_frame_influence.py \
...
Leave-one-frame-out проверка этой слабой серии даёт медианное изменение центра
`31 mas` и максимум `76 mas`. Даже кадр с локальным звёздным RMS `1843 mas`
сдвигает итог только на `15 mas`, тогда как альтернативный покадровый fit сильно
зависит от PSF/loss (`217/123 mas`) и QC не проходит. Поэтому постфактум удалять
один «плохой» кадр для уменьшения ошибки нельзя.
...
Два блока по пять кадров разделены перенаведением примерно на `40 px`; их
отдельные центры расходятся на `181 mas`, а локальные контрольные звёзды — на
медианные `57 mas`. Вариации clipping и порядка интерполяции меняют общий центр
не более чем на `8 mas`, поэтому источник большой ошибки находится в малой
выборке и локальной PSF/привязке, а не в алгоритме сложения.
...
`(37.9,190.3)`, `(−23.6,42.4)` и `(217.1,195.1) mas` по ξ/η.
...
`O−C = (−21.64, +26.56) mas` относительно линейной исходной эфемериды;
формальные ошибки равны `19.1` и `21.3 mas`. Совокупный fitted S/N равен `68.4`,
ни один кадр не упёрся в границу поиска. Повтор относительно второй близкой
звезды изменяет положение на `17.48 mas`, а leave-one-group-out разброс
составляет `15.6/14.4 mas` по ξ/η.

Для каждого из 92 кадров PSF построена независимо по 23–24 звёздам; групповая
PSF сохранена как контроль. Два метода различаются на `34.03 mas` по положению.
Для второй близкой Gaia-звезды та же проверка даёт `14.36 mas`; направления
векторов почти совпадают, но сами векторы расходятся на `19.83 mas`. Это
подтверждает PSF-систематику, но не позволяет вычесть её как общую поправку.
Линейный least-squares выбран вместо `soft_l1`: он уменьшает ошибку и расхождение
двух опорных звёзд, а положения двух loss-функций различаются лишь на `3.61 mas`.
Оба вектора добавляются внешними произведениями к формальной ковариации.
Итоговые ошибки для орбитального fit равны `32.1/31.0 mas`, корреляция ξ/η
равна `−0.601`. Выбор PSF и loss-функции не скрыт внутри формальной ошибки.
...
опорной звезды. Gaia-привязка теперь включает физически допустимый параллакс,
перспективное ускорение при наличии лучевой скорости и топоцентрическое
положение станции 217; шумные почти нулевые параллаксы получают линейный
fallback. Эта поправка сдвигает лучшую координату лишь примерно на
`1.2/0.3 mas`. Поэтому в совместный fit
...
`appulse_systematics_*`, `appulse_psf_comparison_*` и
`appulse_loss_comparison_*` должны проверяться вместе с
итоговой строкой.
...
остаток равен примерно `(−23.6, +42.4) mas`; это согласование одной эпохи, а не поиск
